...
<div class="px-4 text-left text-sm text-gray-900 col-span-12 grid grid-cols-12">
    ...
    <div class="col-span-4 px-20 grid-cols-1 grid">
        ...
        <a class="font-bold {{$resumeText}}" href="{{$selectedPersonal->linked_in}}">Linked In</a>
        @endif
        @if (isset($selectedPersonal->github))
        <a class="font-bold {{$resumeText}}" href="{{$selectedPersonal->github}}">GitHub</a>
        @endif
        @if (isset($selectedPersonal->behance))
        <a class="font-bold {{$resumeText}}" href="{{$selectedPersonal->behance}}">Behance</a>
        @endif
        @if (isset($selectedPersonal->instagram))
        <a class="font-bold {{$resumeText}}" href="{{$selectedPersonal->instagram}}">Instagram</a>
        @endif
        @if (isset($selectedPersonal->facebook))
        <a class="font-bold {{$resumeText}}" href="{{$selectedPersonal->facebook}}">Facebook</a>
        @endif
    </div>
</div>
{{-- Summary --}}
...
<div class="px-4 py-3.5 text-left text-sm text-gray-900 col-span-12">
    <h2 class="text-xl {{$resumeText}} font-bold">Summary</h2>
    <div class="border w-full {{$resumeBorder}}"></div>
...
{{-- Experiences --}}
   ...
   <div class="px-4 py-3.5 text-left text-sm text-gray-900 col-span-12">
       <h2 class="text-xl {{$resumeText}} font-bold">Experiences</h2>
       <div class="border w-full {{$resumeBorder}}"></div>
   ...
   <div class="px-4 py-3.5 text-left text-sm text-gray-900 col-span-12">
   <h2 class="text-xl {{$resumeText}} font-bold">Educations & Qualifications</h2>
   <div class="border w-full {{$resumeBorder}}"></div>
   ...
   <div class="px-4 py-3.5 text-left text-sm text-gray-900 col-span-12">
       <h2 class="text-xl {{$resumeText}} font-bold">Projects & Qualifications</h2>
       <div class="border w-full {{$resumeBorder}}"></div>
   ...
   <div class="px-4 py-3.5 text-left text-sm text-gray-900 col-span-12">
   <h2 class="text-xl {{$resumeText}} font-bold">Skills</h2>
   <div class="border w-full {{$resumeBorder}}"></div>
   ...
   <div class="mt-4 grid grid-cols-12">
       ...
       <div class="my-auto col-span-4">
           <div class=" rounded-full bg-gray-200">
               ...
               <div class="border-4 rounded-full {{$resumeBorder}}" style="width: 25%"></div>
               @endif
               @if ($skill->level == 'Intermediate')
               <div class="border-4 rounded-full {{$resumeBorder}}" style="width: 50%"></div>
               @endif
               @if ($skill->level == 'Advanced')
               <div class="border-4 rounded-full {{$resumeBorder}}" style="width: 75%"></div>
               @endif
               @if ($skill->level == 'Expert')
               <div class="border-4 rounded-full {{$resumeBorder}}" style="width: 100%"></div>
               @endif
           </div>
       </div>
   </div>
   ...
   <div class="px-4 py-3.5 text-left text-sm text-gray-900 col-span-12">
   <h2 class="text-xl {{$resumeText}} font-bold">Languages</h2>
   <div class="border w-full {{$resumeBorder}}"></div>
